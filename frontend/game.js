...
let officeNameFromServer = '海辛小龙虾的办公室';
...
async function loadMemo() {
  ...
  try {
    const response = await fetch('/api/memo?t=' + Date.now(), { cache: 'no-store' });
    const data = await response.json();
...
function updateLoadingProgress() {
  ...
  if (loadingText) {
    loadingText.textContent = `正在加载 OpenClaw 龙虾办公室... ${percent}%`;
  }
}
...
// 状态控制栏函数（OpenClaw Dashboard 暂不支持手动设置状态）
function setState(state, detail) {
  console.log('Set state not implemented in OpenClaw Dashboard:', state, detail);
}
...
function create() {
  ...
  const plaqueText = game.add.text(plaqueX, plaqueY, officeNameFromServer, {
...
function fetchStatus() {
  // OpenClaw Dashboard: 状态从 /api/agents 获取多 Agent 状态
  // 主 Agent 状态显示简化处理，保持函数存在但空转
}
...
function fetchAgents() {
  fetch('/api/agents?t=' + Date.now(), { cache: 'no-store' })
    ...
    .then(data => {
      // OpenClaw Dashboard API 返回格式：{ ok: true, agents: [...], count: N }
      ...
      // 移除不再存在的 agent
      const currentIds = new Set(agentsList.map(a => a.id));
...
function renderAgent(agent) {
  ...
  // 颜色（根据状态）
  const bodyColor = AGENT_COLORS[state] || AGENT_COLORS.default;
